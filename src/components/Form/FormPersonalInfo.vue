    ...
    <nav class="w-full flex items-center">
      <div class="w-full flex justify-end gap-4">
        <router-link :to="{ name: 'builder' }"
          class="bg-slate-800 w-28 p-3 rounded-lg grid place-items-center capitalize">volver</router-link>
        <router-link :to="{ name: 'contact-forms' }"
          class="bg-slate-800 w-28 px-3 py-2 rounded-lg grid place-items-center capitalize">siguiente</router-link>
      </div>
    </nav>
  </footer>
</template>
